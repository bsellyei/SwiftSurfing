...
//  SwiftSurfing
//
//  Created by Séllyei Bence on 2021. 09. 22..
//
// Source from https://github.com/peterent/MultiDatePicker
...
class MDPModel: NSObject, ObservableObject {
    ...
    private var notSelectableDates: [Date] = []
    ...
    convenience init(dateRange: Binding<ClosedRange<Date>?>,
        ...
        self.notSelectableDates = notSelectable
        self.selectionType = includeDays
        self.minDate = minDate
        self.maxDate = maxDate
...
extension MDPModel {
    ...
    private func isEligible(date: Date?) -> Bool {
        ...
        for notSelectable in notSelectableDates {
            let notSelectableComponents = Calendar.current.dateComponents([.year, .month, .day], from: notSelectable)
            if notSelectableComponents.year == dateComponents.year &&
               notSelectableComponents.month == dateComponents.month &&
               notSelectableComponents.day == dateComponents.day
            {
                return false
            }
        }
